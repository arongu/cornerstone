...
function echoHelp(){
    ...
    echo -e "${0} host=db100 type=accounts name=accounts password=postgres_password"
}

function echoFinalStep(){
    echo -e "\n\nComplete!\nNow run flyway to start the database migration!\nGo inside the project dir db/accounts or db/work then execute the following command:\nmvn flyway:migrate -P<profile>"
}
...
function resetDB(){
    ...
    case "${DB_TYPE}" in
        ...
        accounts)
            fileName='accounts_init.sql'
            ./00_gen_init_sql.sh db="${DB_NAME}" sch=accounts > "${fileName}"
            executePsqlFile "${fileName}"
        ;;

        *)
          >&2 echo "Invalid database type: '${keyword}' (accounts/work)"
          exit 3
        ;;
    esac
