...
public class KeyRotationTaskTest {
    ...
    public void run_shouldGoThrowAllTheSteps_whenCalled() throws PublicKeyStoreException {
        ...
        final KeyRotationTask keyRotationTask = new KeyRotationTask(local_key_store, public_key_store, given_rsa_ttl, given_node_name);
        keyRotationTask.run();



        verify(local_key_store, times(1)).setPublicAndPrivateKeys(any(UUID.class), any(PrivateKey.class), any(PublicKey.class));
        verify(public_key_store, times(1)).addKey(any(UUID.class), eq(given_node_name), eq(given_rsa_ttl), anyString());
